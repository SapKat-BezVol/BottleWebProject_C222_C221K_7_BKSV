...
import base64
import io
import os  # Для проверки предотвращения создания директории при моке _save_html_file

# Импорт тестируемых функций
# Пути импорта могут отличаться в зависимости от структуры проекта.
...
class TestPlotGenerator(unittest.TestCase):
    ...
    @patch('services.plot_generator.os.makedirs')
    @patch('services.plot_generator.open', new_callable=unittest.mock.mock_open)
    @patch('services.plot_generator.uuid.uuid4')
    ...
    @patch('services.plot_generator._save_html_file')
    ...
    def test_build_plot_html_hist(self, mock_save_html):
        ...
        self.assertTrue(html_output.endswith("<footer>Generated by Mister Semya</footer>"))
        self.assertEqual(mock_save_html.call_count, 3)
