...
* **Володин (Вариант 4)** – обучение модели (например, линейной регрессии или другого алгоритма) на данных и прогнозирование целевой переменной.
...
* **Страница Вариант 2 (Катаван):**
  На странице создаётся корреляционная матрица для всех или выбранных числовых колонок. Результат выводится в табличном виде и в виде тепловой карты (heatmap) с цветовой шкалой. Пользователь может динамически выбирать подмножество столбцов для анализа.
...
* **Страница Вариант 4 (Володин):**
  Реализовано обучение простой модели (например, линейной регрессии) на предоставленных данных. Пользователь выбирает целевую колонку и, возможно, параметры модели. После обучения отображаются метрики качества (R^2, MSE) и построенный прогноз (график истинных vs предсказанных значений). Можно предложить загрузку новых данных для прогнозирования.
...
*(Файл `requirements.txt` должен содержать перечисленные библиотеки.)*

## Запуск проекта

1. **Запуск сервера:** Перейдите в корневую папку проекта и выполните:
...
2. **Ввод данных:** На каждой странице приложения есть форма для загрузки данных. Можно загрузить CSV-файл с данными (не более 10 столбцов, до 1000 строк). Пример формата CSV:
...
3. **Навигация по страницам:** Главная страница (`/`) предлагает ссылки на четыре варианта анализа. Перейдите по нужной ссылке, загрузите данные и нажмите кнопку «Анализ» или «Построить график».
...
(если проект настроен на использование `pytest`).
